...
我们有一个ATM，当我们处于checkPin的状态时我们会要求用户从外部输入Pin，然后检查Pin是否正确，如果正确那么进入到Successed 对应的状态，如果错误进入到Failed的状态。一个常见的要求是：用户最多尝试输入三次Pin，如果三次都错误那么就应该退卡，并返回到开始的界面。这里的三次是一个及其重要的安全参数，不应该被改变。
...
第二个：[text](https://github.com/sdzx-1/ray-game/blob/master/src/select.zig)
...
```
选择的具体行为由三个通用状态和十个消息一同构成。这些状态和消息实现了：通过鼠标选择一个元素，以及鼠标悬停时应该如何响应。
在ray-game这个项目中选择语义被重用了至少八次，这极大的减少了代码，提高了正确性。

这个项目中一个有趣的例子：你需要选择一个建筑放置到网格中，我称之为两阶段选择。需要两次选择：第一次选择一个建筑，第二次选择一个位置，同时建筑的选择会限制位置的选择。![select_twict](data/select_twice.gif)
...
```
由placeST提供选择需要的几个具体函数，毕竟placeST需要两阶段选择的建筑和位置来具体放置建筑，所以placeST 知道什么是正确的建筑和位置。
这段代码是及其简洁的描述了你的想法，但如果你查看状态图你会发现它的实际状态很复杂. ![graph](data/graph.png)
通过简单的声明我们嵌套的复用了复杂的选择语义代码，这是巨大的胜利！！ [这一切的完整代码就在这里，大约130行代码。](https://github.com/sdzx-1/ray-game/blob/587f1698cb717c393c3680060a057ac8b02d89c2/src/play.zig#L33)
